        ...
        for (final Entry<String, AbstractUserCommand> entry : commands.entrySet()) {
            if (commandName.startsWith(entry.getKey())) {
                if (!entry.getValue().getAllowedAccountTypes().contains(player.getAccount().getAccountType())) {
                    player.getConnection().sendPacket(new SM_PLAYER_CHAT(player, I18nService.getInstance().translate(player.getAccount().getLocale(), UserErrorMessageEnum.NOT_ALLOWED_ACCOUNT_TYPE.key), ChatTypeEnum.SYSTEM));
                    return true;
                }
