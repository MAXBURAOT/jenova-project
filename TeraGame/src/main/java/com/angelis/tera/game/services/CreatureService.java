...
public class CreatureService extends AbstractService {
    ...
    private final List<Long> creaturesExperiences = new FastList<>();
    ...
    public void onInit() {
        ...
        for (int i = 0; i < 100; i++) {
            creaturesExperiences.add((long) (a * i * i + b * i + c));
        }
    ...
    public void moveCreature(final Creature creature, final WorldPosition newWorldPosition) {
    ...
    public long getExperience(final int level) {
        if (!this.creaturesExperiences.contains(level)) {
            return 0;
        }

        return this.creaturesExperiences.get(level);
    }
