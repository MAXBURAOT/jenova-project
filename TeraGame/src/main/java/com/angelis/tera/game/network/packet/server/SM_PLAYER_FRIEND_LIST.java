...
public class SM_PLAYER_FRIEND_LIST extends TeraServerPacket {
    ...
    protected void writeImpl(final TeraGameConnection connection, final ByteBuffer byteBuffer) {
        ...
        final int friendSize = this.player.getFriends().size();
        writeH(byteBuffer, (short) friendSize);

        if (friendSize > 0) {
            writeH(byteBuffer, 8);
            int i = 0;
            for(final PlayerRelation playerRelation : this.player.getFriends()) {
                final Player friend = playerRelation.getPlayer();
                ...
                writeB(byteBuffer, friend.getZoneData());
                ...
                this.writeBufferPosition(byteBuffer, shift + 4, Size.H);
                writeH(byteBuffer, 0);
