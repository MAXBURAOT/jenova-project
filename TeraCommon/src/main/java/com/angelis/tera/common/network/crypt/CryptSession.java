...
public final class CryptSession implements IsObservable<EmptyEventable> {
    ...
    public CryptSession(final byte[] clientKey1, final byte[] clientKey2, final byte[] serverKey1, final byte[] serverKey2) {
    ...
    public final void readKeyPacket(byte[] data) {
        switch (cryptState) {
            case KEY1:
    ...
    public final void decrypt(final byte[] data, int size) {
    ...
    public final void encrypt(final byte[] data, int size) {
        if (this.cryptState != CryptState.CRYPTED) {
            return;
        }
    ...
    @Override
    public void addObserver(IsObserver<EmptyEventable> observer) {
        this.observers.add(observer);
    }

    @Override
    public void notifyObservers(EmptyEventable event, Object... data) {
        for (IsObserver<EmptyEventable> observer : this.observers) {
