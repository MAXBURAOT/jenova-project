...
public class Sha {
    protected int[] MessageDigest = { 0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476, 0xC3D2E1F0, };

    protected long LengthLow;
    protected long LengthHigh;
    ...
    protected long CircularShift(int bits, long word) {
    ...
    protected void ProcessMessageBlock() {
        long[] k = // Constants defined in SHA
        ...
        for (t = 0; t < 16; t++) {
            w[t] = ((long) MessageBlock[t * 4]) << 24;
            w[t] |= ((long) MessageBlock[t * 4 + 1]) << 16;
            w[t] |= ((long) MessageBlock[t * 4 + 2]) << 8;
        ...
        MessageDigest[0] = (int) ((MessageDigest[0] + a) & 0xFFFFFFFF);
        MessageDigest[1] = (int) ((MessageDigest[1] + b) & 0xFFFFFFFF);
        MessageDigest[2] = (int) ((MessageDigest[2] + c) & 0xFFFFFFFF);
        MessageDigest[3] = (int) ((MessageDigest[3] + d) & 0xFFFFFFFF);
        MessageDigest[4] = (int) ((MessageDigest[4] + e) & 0xFFFFFFFF);
    ...
    protected void PadMessage() {
         ...
         * initial padding bits and length. If so, we will pad the block,
    ...
    public static int[] Digest(byte[] src) {
        Sha sha = new Sha();
        sha.Input(src);
        sha.Result();
