...
/**
 * This class represents a packet format in the protocol definition tree. It is
 * a container for {@link PacketParts PacketParts}
 * 
 * @author Gilles Duboscq TODO: find a way to ensure part IDs unicity
 */
public class PacketFormat extends ProtocolNode {
    ...
    public String getPartsStr() {
        if (_partStr == null) {
            ...
            TextBuilder tb = new TextBuilder();
            tb.append("(");
            tb.append(Util.makeFormatTypeString(super.getIdParts()));
            tb.append(") ");
            tb.append(Util.makeFormatPartString(this.getDataFormat().getMainBlock().getParts()));
    ...
    /**
     * format : (c) chdfQSsbx[ddd] ^id ^for id is mandatory, spaces are allowed
     * 
     * @param str
     * @param ids
     * @return
