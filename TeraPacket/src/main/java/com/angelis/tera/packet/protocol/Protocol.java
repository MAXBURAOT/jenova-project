...
import com.angelis.tera.packet.parser.formattree.ForPart;
import com.angelis.tera.packet.parser.formattree.Part;
import com.angelis.tera.packet.parser.formattree.PartContainer;
import com.angelis.tera.packet.parser.formattree.SwitchCaseBlock;
import com.angelis.tera.packet.parser.formattree.SwitchPart;
import com.angelis.tera.packet.parser.parttypes.RawPartType;
import com.angelis.tera.packet.parser.valuereader.Reader;
import com.angelis.tera.packet.protocol.protocoltree.PacketFamilly;
...
import com.angelis.tera.packet.protocol.protocoltree.ProtocolNode;
...
public class Protocol {
    ...
    private int _sizeBytes; // this is the number of bytes used to specify the
                            // size of packets (2 is here as a default value
                            // that will be changed if specified)
    ...
    public Protocol(int sizeBytes, String name, int port, String encryption) {
        ...
        _clientPackets.setDirection(PacketDirectionEnum.CLIENT);
        _serverPackets = new PacketFamilly("ServerPackets");
        _serverPackets.setDirection(PacketDirectionEnum.SERVER);
    }
    ...
    public void saveProtocol(String fileName) {
        try {
            ...
            if (_serverPackets != null) {
                ...
                MainPacket.getUserInterface().log("Server Packets were not loaded, skipped while saving");
            }
            ...
            if (_clientPackets != null) {
                ...
                MainPacket.getUserInterface().log("Client Packets were not loaded, skipped while saving");
            }
    ...
    public PacketFormat getFormat(DataPacket packet, PacketId _packetid) {
        if (packet.getDirection() == PacketDirectionEnum.SERVER) {
            if (_serverPackets == null)
                return null;
        ...
        else if (packet.getDirection() == PacketDirectionEnum.CLIENT) {
    ...
    public List<DataStructure> parseFile(String fileName) throws IOException {
        ...
        List<DataStructure> list = new FastList<>();

        while (fis.available() > 0) {
            PacketDirectionEnum dir;
            if (fis.read() == 0) {
                dir = PacketDirectionEnum.CLIENT;
            }
            else {
                dir = PacketDirectionEnum.SERVER;
            }
    ...
    private void loadProtocol(String pFile) {
        ...
        try {
            ...
            if (!file.exists()) {
                ...
                _serverPackets = new PacketFamilly("ServerPackets");
                _serverPackets.setDirection(PacketDirectionEnum.SERVER);
                MainPacket.getUserInterface().log("Protocol file didnt exist (" + pFile + "), creating new protocol.");
                _filename = null;
                return;
            }
            ...
            if (!root.getNodeName().equals("protocol")) {
                MainPacket.getUserInterface().log("Error malformed protocol : root node should be called 'protocol'.");
                return;
            }
            ...
            if (node != null) {
                try {
                    ...
                    MainPacket.getUserInterface().log("Warning: The sizebyte attribute of protocol must be an integer. Couldnt read the attribute provided, falling back to the default value");
                    _sizeBytes = 2;
                }
            }
            else {
            ...
            if (node != null) {
                try {
                    ...
                    MainPacket.getUserInterface().log("Warning: The port attribute of protocol must be an integer. Couldnt read the attribute provided");
                }
            }
            ...
            if (node != null) {
                ...
                MainPacket.getUserInterface().log("Error while loading " + pFile + " : protocol has no 'name' attribute");
                return;
            }
                ...
                if ("packetfamilly".equalsIgnoreCase(n.getNodeName())) {
                    ...
                    if (familly != null) {
                        switch (familly.getDirection()) {
                            ...
                            break;
                            default:
                                MainPacket.getUserInterface().log("Error packetfamilly doesnt have a valid way");
                        }
                    }
                    else {
                        MainPacket.getUserInterface().log("Error packetfamilly returned is null there was an error");
                    }
                }
            }
            _loaded = true;
            ...
            MainPacket.getUserInterface().log("Error while parsing protocol " + pFile);
            e.printStackTrace();
        }
    }
    ...
    private PacketFamilly parseFamilly(Node n, boolean isRoot) {
        ...
        if (isRoot) {
            ...
            if (atr == null) {
                MainPacket.getUserInterface().log("Error, packetfamilly is not root and has no id. skipping it:");
                return null;
            }
        ...
        if (atr == null) {
            if (isRoot) {
                MainPacket.getUserInterface().log("Error, Root packetfamilly doesnt have 'way'. skipping it");
                return null;
            }
        }
        else {
            ...
            if (way.equals("ServerPackets")) {
                newFamilly.setDirection(PacketDirectionEnum.SERVER);
            }
            else if (way.equals("ClientPackets")) {
                newFamilly.setDirection(PacketDirectionEnum.CLIENT);
            }
        }
        // --------
        atr = attrs.getNamedItem("switchtype");
        if (atr == null) {
            MainPacket.getUserInterface().log("Error, packetfamilly doesnt have 'switchtype'. skipping it");
            return null;
        }
    ...
    private PacketFormat parseFormat(Node n) {
        ...
        atr = attrs.getNamedItem("id");
        if (atr == null) {
            MainPacket.getUserInterface().log("Error, packet doesnt have 'id'. skipping it");
            return null;
        }
        ...
        atr = attrs.getNamedItem("name");
        if (atr == null) {
            MainPacket.getUserInterface().log("Warning, packet doesnt have 'name'");
            name = "";
        }
        else {
        ...
        try {
            ...
            MainPacket.getUserInterface().log("Error: parsing format: " + newFormat.toString() + " - Details:  " + e.getMessage());
        }
    ...
    private boolean parseParts(Node n, PartContainer pc) {
        ...
        for (Node o = n.getFirstChild(); o != null; o = o.getNextSibling()) {
            if ("part".equalsIgnoreCase(o.getNodeName())) {
                ...
                atr = attrs.getNamedItem("id");
                if (atr == null) {
                    MainPacket.getUserInterface().log("Error, for doesnt have 'id'. skipping packet");
                    return false;
                }
                ...
                atr = attrs.getNamedItem("id");
                if (atr == null) {
                    MainPacket.getUserInterface().log("Error, switch doesnt have 'id'. skipping packet");
                    return false;
                }
                ...
                for (Node caseNode = o.getFirstChild(); caseNode != null; caseNode = caseNode.getNextSibling()) {
                    if ("case".equalsIgnoreCase(caseNode.getNodeName())) {
                        ...
                        atr = attrs.getNamedItem("id");
                        if (atr == null) {
                            MainPacket.getUserInterface().log("Error, case doesnt have 'id'. skipping packet");
                            return false;
                        }
                        ...
                        if (atr.getNodeValue().equalsIgnoreCase("default")) {
                            ...
                            try {
                                ...
                                MainPacket.getUserInterface().log("Warning, case doesnt have a valid 'id'. making it default");
                                newSwitchCase = new SwitchCaseBlock(newSwitchBlock, analyzerName);
                            }
                        }
                        if (comment != null)
    ...
    private Part parsePart(Node n, PartContainer pc) {
        ...
        if (atr == null) {
            MainPacket.getUserInterface().log("Warning, part doesnt have 'name'");
            partName = "";
        }
        else {
        ...
        if (atr == null) {
            ...
            try {
                ...
                MainPacket.getUserInterface().log("Warning: parts id must be an integer");
                partId = -1;
            }
        }
        ...
        if (atr == null) {
            MainPacket.getUserInterface().log("Error, part doesnt have 'type'. skipping packet");
            return null;
        }
        ...
        for (Node subNode = n.getFirstChild(); subNode != null; subNode = subNode.getNextSibling()) {
            if ("comment".equals(subNode.getNodeName())) {
                if (partComment != null) {
                    MainPacket.getUserInterface().log("Warning, part '" + (partName) + "' has mutiple comments");
                }
                partComment = subNode.getFirstChild().getNodeValue();
            }

            if ("reader".equals(subNode.getNodeName())) {
                if (r != null)
                    MainPacket.getUserInterface().log("Warning, part '" + (partName) + "' has mutiple readers");
                NamedNodeMap attrs2 = subNode.getAttributes();
                atr = attrs2.getNamedItem("type");
                if (atr == null) {
                    MainPacket.getUserInterface().log("Warning, part '" + (partName) + "' has a reader with no type");
                    continue;
                }
                ...
                Class<?> clazz = null;
                try {
                    clazz = Class.forName("com.angelis.tera.packet.parser.valuereader." + atr.getNodeValue() + "Reader");
                }
                ...
                if (clazz == null) {
                    MainPacket.getUserInterface().log("Warning, part '" + (partName) + "' reader's could not be found in either parser or custom packages");
                    continue;
                }
                try {
                    r = (Reader) clazz.newInstance();
                    if (!r.loadReaderFromXML(subNode)) // drop reader if loading
                                                       // went wrong
                        r = null;
    ...
    public static void report(String severity, SAXParseException e) {
        MainPacket.getUserInterface().log(severity + ": " + e.getMessage() + " (Line " + e.getLineNumber() + ", Column: " + e.getColumnNumber() + ")");
    }
