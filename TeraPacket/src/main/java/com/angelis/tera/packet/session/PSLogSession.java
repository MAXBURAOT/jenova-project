/**
 * 
 */
package com.angelis.tera.packet.session;

import java.io.IOException;
import java.net.Inet4Address;
import java.net.InetAddress;

import com.angelis.tera.packet.MainPacket;
import com.angelis.tera.packet.logwriters.PSLogWriter;
import com.angelis.tera.packet.protocol.Protocol;

/**
 * Basically a TCPSession that autmatically logs the packets on the fly.
 * 
 * @author Ulysses R. Ribeiro
 * 
 */
public class PSLogSession extends TCPSession {

    private final PSLogWriter _logWriter;

    public PSLogSession(final long sessionId, final Protocol protocol, final String prefix, final boolean crypted, final InetAddress serverAddr, final InetAddress clientAddr) throws IOException {
        super(sessionId, protocol, prefix, crypted, MainPacket.DECRYPT_ACTIVE);
        this.setClientIp((Inet4Address) clientAddr);
        this.setServerIp((Inet4Address) serverAddr);
        _logWriter = new PSLogWriter(this, true);
    }

    @Override
    public void addPacket(final byte[] data, final boolean fromServer, final long time) {
        // let it be added normally
        super.addPacket(data, fromServer, time);
        // notify writer
        try {
            this.getLogWriter().writePacket(this.getPackets().getLast());
        }
        catch (final IOException e) {
            MainPacket.getUserInterface().log("ERROR: While writing packet data on Session ID: " + this.getSessionId() + ", attempting to continue.");
        }
    }

    @Override
    public void saveSession() {
        // we are already saving the session on the fly
        // prevent the super method from doing it again
    }

    @Override
    public void close() {
        super.close();
        try {
            this.getLogWriter().close();
        }
        catch (final IOException e) {
            MainPacket.getUserInterface().log("ERROR: While closing log of Session ID: " + this.getSessionId() + ".");
        }
    }

    protected PSLogWriter getLogWriter() {
        return _logWriter;
    }

}
