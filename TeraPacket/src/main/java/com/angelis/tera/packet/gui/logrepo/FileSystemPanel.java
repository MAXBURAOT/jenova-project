...
import com.angelis.tera.packet.gui.ViewTab;
import com.angelis.tera.packet.gui.images.IconsTable;
import com.angelis.tera.packet.logrepo.LogFile;
import com.angelis.tera.packet.logrepo.LogRepository;
...
public class FileSystemPanel extends JPanel implements ActionListener
{
    ...
    public void addLogFile(LogFile logFile)
    {
        ...
        {
            // GO IN PANIC
        	MainPacket.getUserInterface().log("Warning : Unable to find proper category for log file "+logFile.getName());
        	return;
        }
        ...
        private void showPopup(MouseEvent e)
        {
            ...
            else if (e.getClickCount() == 2 && e.getButton() == MouseEvent.BUTTON1)
            {
                ...
                if (tp != null && (tp.getLastPathComponent()) instanceof LogFileNode)
                {
                    ...
                    LogFilesTab logFilesTab = ((Main) MainPacket.getUserInterface()).getLogRepoTab().getLogFilesTab();
            ...
            if (cmd.equals("open"))
            {
                Thread t = new Thread()
                {
                    ...
                    public void run()
                    {
                        ...
                        if (logFile.isLocal())
                        {
                            ((Main) MainPacket.getUserInterface()).openSession(logFile.getFile());
                        }
                    }
                };
                t.start();
            ...
            else if (cmd.equals("opengo"))
            {
                Thread t = new Thread()
                {
                    ...
                    public void run()
                    {
                        ...
                        {
                            ((Main) MainPacket.getUserInterface()).openSession(logFile.getFile());
                            ViewTab vt = ((Main) MainPacket.getUserInterface()).getViewerTab();
                            vt.setSelectedIndex(vt.getComponentCount()-1);
                            ((Main) MainPacket.getUserInterface()).getTabPane().setSelectedComponent(vt);
                        }
                    }
                };
                t.start();
