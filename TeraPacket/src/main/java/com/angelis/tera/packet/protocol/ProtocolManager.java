...
import com.angelis.tera.packet.gui.ChoiceDialog;
...
public class ProtocolManager {
    ...
    private static class SingletonHolder {
        private final static ProtocolManager singleton = new ProtocolManager(MainPacket.getConfigProperty("protocols", "./protocols"));
    }
    ...
    public Set<Protocol> getProtocolForPort(int port) {
        if (!_protocolsByPort.containsKey(port))
            MainPacket.getUserInterface().log("ProtocolManager : Warning : can not find protocol for port " + port);
        return _protocolsByPort.get(port);
    }

    public Protocol getProtocolByName(String name) {
        if (!_protocolsByName.containsKey(name))
            MainPacket.getUserInterface().log("ProtocolManager : Warning : can not find protocol for name " + name);
    ...
    public void loadProtocols() {
        ...
        if (!dir.isDirectory()) {
            MainPacket.getUserInterface().log("Invalid Protocols directory (" + _protocolsDir + ")");
            return;
        }
        ...
        for (File f : files) {
            ...
            if (p.isLoaded()) {
                MainPacket.getUserInterface().log("Loaded protocol [" + p.getName() + "] from (" + f.getName() + ")");
                ...
                if (_protocolsByName.put(p.getName(), p) != null) {
                    MainPacket.getUserInterface().log("ProtocolManager : Warning : there is more than one protocol for the name " + p.getName() + " only the last one will be available through getProtocolByName.");
                }
            }
        }
    }
