...
import com.angelis.tera.packet.gui.logrepo.LogRepoTab;
import com.angelis.tera.packet.gui.protocoleditor.ProtocolEditor;
import com.angelis.tera.packet.logreaders.AbstractLogReader;
import com.angelis.tera.packet.session.DataPacket;
import com.angelis.tera.packet.session.GameSessionTable;
import com.angelis.tera.packet.session.Session;
import com.angelis.tera.packet.utils.NpcGatherExporter;
import com.angelis.tera.packet.utils.NpcInfoExporter;
import com.angelis.tera.packet.utils.NpcSpawnExporter;
...
public class Main implements IUserInterface {
	...
	public void init() {
		_frame = new JFrame("Packet Samurai [tera-emu edition]");
		...
		_frame.setExtendedState(Frame.MAXIMIZED_HORIZ);
		...
		MouseListener mouseListener = new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					MainPacket.setConfigProperty("NetworkInterface", Integer.toString(index));
					_selectInterfaceWindow.dispose();
				}
			}
		};
	...
	public void searchNext() {
		if (getViewerTab().getComponentCount() > 0) {
			if (_searchDlg != null) {
				ViewPane pane = ((Main) MainPacket.getUserInterface()).getViewerTab().getCurrentViewPane();
				if (pane != null) {
					int index = _searchDlg.search(pane.getSelectedPacketindex() + 1);
					if (index >= 0) {
						JTable pt = ((Main) MainPacket.getUserInterface()).getViewerTab().getCurrentViewPane().getPacketTable();
	...
	public class MenuActionListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent ev) {
			...
			if (actionCmd.equals("Open")) {
				final JFileChooser chooser = new JFileChooser(MainPacket.getConfigProperty("lastLogDir", "."+File.separator+"logs"+File.separator));
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					Thread t = new Thread() {
						@Override
                        public void run() {
							((Main) MainPacket.getUserInterface()).openSession(chooser.getSelectedFile());
						}
					};
					t.start();
				}
	...
	public void openSession(File file) {
		...
		try {
			...
			MainPacket.getUserInterface().log("ERROR: Opening (" + file.getAbsolutePath() + "), file was not found.");
		} catch (IOException ioe) {
			ioe.printStackTrace();
			MainPacket.getUserInterface().log("ERROR: Opening (" + file.getAbsolutePath() + "), I/O error.");
		} catch (BufferUnderflowException e) {
			MainPacket.getUserInterface().log("ERROR: Opening (" + file.getAbsolutePath() + "), file format error.");
			e.printStackTrace();
		}

		if (sessions != null) {
			MainPacket.setConfigProperty("lastLogDir", file.getParent());
