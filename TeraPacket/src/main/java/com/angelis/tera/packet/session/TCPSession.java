...
import com.angelis.tera.packet.session.packetbuffer.DefaultPacketBuffer;
import com.angelis.tera.packet.session.packetbuffer.PacketBuffer;
...
public class TCPSession extends Session {
    ...
    private void initBuffers() {
        ...
        Class<?> clazz = null;
        try {
            clazz = Class.forName("com.angelis.tera.packet.session.packetbuffer." + pBufType);
            if (clazz == null) {
                MainPacket.getUserInterface().log("Wrong PacketBuffer type in " + this.getProtocol().getFileName() + " defaulting to DefaultPacketBuffer");
        ...
        else // from client
        {
            ...
            for (TCPPacket packet : _scTCPPacketBuffer.getSequencedPackets()) {
                ...
                if (packet.fin || packet.rst) {
                    MainPacket.getUserInterface().log("[S -> C] TCP Sequencing Buffer Report: Pending: " + _scTCPPacketBuffer.getPendingSequencePackets());
                    MainPacket.getUserInterface().log("[S -> C] TCP Data Buffer Report: Pending: " + _scPacketBuffer.nextAvaliablePacket());
                    MainPacket.getUserInterface().log("[C -> S] TCP Sequencing Buffer Report: Pending: " + _csTCPPacketBuffer.getPendingSequencePackets());
                    MainPacket.getUserInterface().log("[C -> S] TCP Data Buffer Report: Pending: " + _csPacketBuffer.nextAvaliablePacket());
                    MainPacket.getUserInterface().log("Connection ended. (FIN: " + packet.fin + " - RST: " + packet.rst + ")");
                    this.saveSession();
                    this.close();
                    return;
                }
