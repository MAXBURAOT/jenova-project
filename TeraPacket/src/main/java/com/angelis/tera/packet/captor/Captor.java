...
public class Captor implements Runnable {
	...
	public void initDevice() throws Exception {
		...
		try {
			...
			MainPacket.getUserInterface().log(
		...
		// if there is only one we bind it
			...
			if (!MainPacket.configExists("NetworkInterface")) {
				selectNetWorkInterface();
			} else {
				int deviceNumber = Integer.parseInt(MainPacket
	...
	public void selectNetWorkInterface() {
		...
		if (ret != null) {
			...
			MainPacket.setConfigProperty("NetworkInterface", Integer
					.toString(ret[0]));
		} else {
			MainPacket.getUserInterface().log("No interface selected.");
		}
	...
	private void configureProtocols() throws Exception {
		Captor.setActiveProtocols(MainPacket.loadSnifferActiveProtocols());
	...
	public void openDevice(NetworkInterface ni) {
		try {
			...
			String filter = MainPacket.getConfigProperty("filter", "")
					.trim();
			if (filter.length() > 0) {
				MainPacket.getUserInterface().log(
	...
	public static Map<Integer, Protocol> getActiveProtocols() {
	...
	public String getPacketDump(TCPPacket tcpPacket) {
		...
		sb.append("Received a packet: " + MainPacket.hexDump(tcpPacket.data)
				+ " - Flags: ");
		sb.append(" - ACK: "
	...
	public static void showSetActiveProtocols() {
		...
		if (ret != null) {
			...
			MainPacket.saveSnifferActiveProtocols();
		}
	}
}
