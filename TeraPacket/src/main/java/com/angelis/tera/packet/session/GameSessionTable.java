...
public class GameSessionTable 
{
	...
	public TCPSession newGameSession(long sessionId, int port, InetAddress serverAddr, InetAddress clientAddr) throws IOException
	{
	    ...
	    if (protocol != null)
	    {
	        ...
	        if (MainPacket.getUserInterface() instanceof Main)
	        {
	            ((Main) MainPacket.getUserInterface()).showSession(gameSession, true);
	        }
	        MainPacket.getUserInterface().log("Started to log new Session (ID: "+gameSession.getSessionId()+")");
	        return gameSession;
	    }
