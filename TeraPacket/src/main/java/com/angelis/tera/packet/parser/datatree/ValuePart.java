package com.angelis.tera.packet.parser.datatree;
...
import com.angelis.tera.packet.parser.DataStructure.DataPacketMode;
...
public class ValuePart extends DataTreeNode
{
    ...
    public void parse(ByteBuffer buf)
    {
        ...
        if (size > 65536)
        {
            if (MainPacket.VERBOSITY.ordinal() >= MainPacket.VerboseLevel.NORMAL.ordinal())
            {
        ...
        if (_bxSize < 0)
        {
            if(getModelPart().isDynamicBSize())
            {
                ...
                if (vp == null || !(vp instanceof IntValuePart))
                    MainPacket.getUserInterface().log("ValuePart: Invalid SizeId for bx: part can not be found or is not an integer");
                else
