...
public class MultiValueReader implements Reader {
    ...
    public boolean loadReaderFromXML(Node n) {
        ...
        for (Node subNode = n.getFirstChild(); subNode != null; subNode = subNode.getNextSibling()) {
            if ("case".equals(subNode.getNodeName())) {
                ...
                if (atr == null) {
                    MainPacket.getUserInterface().log("Error while reading MultiValue reader : no val atribute");
                    return false;
                }
                if ("default".equals(atr.getNodeValue())) {
                    ...
                    if (_enum != null) {
                        ...
                        if (atr == null) {
                            MainPacket.getUserInterface().log("Warning: reading MultiValue reader : no enumval attribute while reader has an enum : disabling enum for this reader");
                            _enum = null;
                        }
                        else {
                ...
                try {
                    ...
                    MainPacket.getUserInterface().log("Error while reading MultiValue reader : val should be a number");
                    return false;
                }
                atr = attrs.getNamedItem("display");
                String disp = atr.getNodeValue();
    ...
    // XXX this is dirty, we should see if we can do something without having
    // dirty unchecked type casting
    @Override
