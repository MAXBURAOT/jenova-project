...
import com.angelis.tera.packet.protocol.PacketId.IdPart;
import com.angelis.tera.packet.session.DataPacket;
import com.angelis.tera.packet.session.GameSessionViewer;
...
public class ViewPane extends JPanel {
    ...
    public ViewPane(GameSessionViewer gsv) {
        ...
        JScrollPane hexDumpScrollPane = new JScrollPane(_hexDumpArea, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
    ...
    public void addStyledText(String text, String style, boolean index) {
        ...
        if (s == null) {
            MainPacket.getUserInterface().log("WARNING: Missing style for partType: " + style);
            style = "base";
        }
    ...
    public FilterDlg getFilterDialog() {
        if (_filterDlg == null)
            _filterDlg = new FilterDlg(((Main) MainPacket.getUserInterface()).getMainFrame(), this);
        return _filterDlg;
    }
    ...
    public JTable getPacketTable() {
        // OMG that's hacky! here to update packet count as we usualy do a
        // getPacketTable() to update the ui after adding packets
    ...
    public void updateCurrentPacket(boolean forced) {
        if (!forced && _currentSelectedPacket == _packetTable.getSelectedRow())
            return;
        /*
         * try { DataStructure gp =
         * _gameSessionViewer.getPacket(_currentSelectedPacket); if(gp!= null)
         * gp.setViewed(false); } catch (IndexOutOfBoundsException e) {
         * 
         * }
         */
        ...
        if (gp.getPacketFormat() != null && gp.isValid()) {
            ...
            // if we know the opcode but the format was invalid still show the
            // opcode
    ...
    public class PacketSelectionListener implements ListSelectionListener {
        ...
        public void valueChanged(ListSelectionEvent e) {
            ...
            else if (e.getSource() == view.getPacketTable().getColumnModel().getSelectionModel() && view.getPacketTable().getColumnSelectionAllowed()) {
            }
        }
    ...
    public class PacketViewTreeRenderer extends DefaultTreeCellRenderer {
        ...
        public Component getTreeCellRendererComponent(JTree tree, Object value, boolean sel, boolean expanded, boolean leaf, int row, boolean hasFocus) {
            ...
            if (value instanceof DataPartNode && leaf) {
                // System.out.println(((DataPartNode)
                // value).getPacketNode().getModelPart());
