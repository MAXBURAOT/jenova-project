package com.angelis.tera.packet.session.packetbuffer;
...
public class DefaultPacketBuffer implements PacketBuffer {
    ...
    public void getNextPacket(byte[] header, byte[] data) {
        ...
        // System.out.println("PacketBuffer : newtPacket : returned one packet size="+(data.length
        // + header.length));
        // System.out.println("PacketBuffer : newtPacket : mark:"+mark);
        // compact the buffer
        _buffer.compact();
        // System.out.println("PacketBuffer : newtPacket : wants to position to :"+(mark
        // - (data.length + header.length))+" and limit : "+_buffer.limit());
    }
